            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <table class="m-4 border-spacing-7">
                        @foreach (json_decode($teacher) as $key => $value)
                        ...
                        <tr>
                            ...
                            <td class="p-2">{{ $teacher->created_at }}</td>
                        </tr>
                        @if($updated_by)
                        <tr>
                            <td class="p-2">{{__('updated by:')}}</td>
                            <td class="my-2"><a href="/users/{{$updated_by->id}}"> {{ $updated_by->name }} </a></td>
                            <td class="p-2"> {{ $teacher->updated_at }} </td>
                        </tr>
                        @endif
                       
                    </table>
                    <a href="/teachers" class="bg-blue-600 rounded text-blue-200 p-2 px-4 mx-4e">{{ __('back')}}</a>
                </div>
            </div>
        </div>
    </div>
